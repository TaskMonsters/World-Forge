    ...
    content.innerHTML = `
      <div style="margin-bottom: 2rem;">
        ...
        <div style="background: var(--bg-secondary); border: 2px solid var(--border-color); border-radius: 0.5rem; padding: 1.25rem; margin-top: 1rem;">
          <h3 onclick="MapBuilder.toggleHowToUse()" style="font-size: 1rem; font-weight: 700; color: var(--text-primary); margin: 0 0 0.75rem 0; cursor: pointer; user-select: none; display: flex; align-items: center; gap: 0.5rem;">
            <span id="howToUseToggle">▼</span> How to use:
          </h3>
          <div id="howToUseContent" style="display: block;">
          ...
          <div style="margin-bottom: 1rem; padding-top: 0.75rem; border-top: 1px solid var(--border-color);">
            ...
            <div style="font-size: 0.875rem; color: var(--text-secondary); line-height: 1.6;">
              ...
              <div style="margin-bottom: 0.25rem;"><strong>Assets:</strong> Click to select • Ctrl/Cmd+Click for multi-select • Drag to move • +/- to resize • R to rotate • Shift+D to duplicate • Delete to remove</div>
      ...
      <div class="map-container">
        <div class="map-toolbar">
          <div class="map-tool-group">
            <button class="map-tool-btn" id="toolUndo" disabled style="opacity: 0.5;">↩️ Undo</button>
          ...
          <div class="map-tool-group" style="margin-left: auto;">
            <div id="assetCounter" style="font-size: 1.125rem; font-weight: 600; color: var(--text-primary); padding: 0.5rem 1rem; background: var(--card-bg); border: 2px solid var(--black); border-radius: 8px; box-shadow: 2px 2px 0 var(--black); white-space: nowrap;">
              <span id="assetCountText">0/50 Assets</span>
            </div>
          </div>
        </div>
        ...
        <div class="emoji-picker-modal" id="backgroundPickerModal" style="display: none;">
          <div class="emoji-picker-content" style="max-width: 600px; max-height: 90vh;">
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; padding: 1.5rem; overflow-y: auto; max-height: calc(90vh - 80px);">
              ...
              <button class="background-option" onclick="MapBuilder.setBackground('newland')">
                <div class="background-preview" style="background: url('assets/backgrounds/newland.png'); background-size: cover;"></div>
                <span>New Land</span>
              </button>
              <button class="background-option" onclick="MapBuilder.setBackground('winterland')">
                <div class="background-preview" style="background: url('assets/backgrounds/winterland.png'); background-size: cover;"></div>
                <span>Winter Land</span>
              </button>
              <button class="background-option" onclick="MapBuilder.setBackground('town')">
                <div class="background-preview" style="background: url('assets/backgrounds/town.png'); background-size: cover;"></div>
                <span>Town</span>
              </button>
              <button class="background-option" onclick="MapBuilder.setBackground('darkfantasy')">
                <div class="background-preview" style="background: url('assets/backgrounds/darkfantasy.png'); background-size: cover;"></div>
                <span>Dark Fantasy</span>
              </button>
              <button class="background-option" onclick="MapBuilder.setBackground('ruraltown')">
                <div class="background-preview" style="background: url('assets/backgrounds/ruraltown.png'); background-size: cover;"></div>
                <span>Rural Town</span>
              </button>
              <button class="background-option" onclick="MapBuilder.setBackground('oldcity')">
                <div class="background-preview" style="background: url('assets/backgrounds/oldcity.png'); background-size: cover;"></div>
                <span>Old City</span>
              </button>
            </div>
            <div style="padding: 1.5rem; border-top: 1px solid var(--border-color);">
              <h4 style="font-size: 0.875rem; font-weight: 600; margin-bottom: 1rem; color: var(--text-primary);">Custom Background</h4>
              <div id="customBackgroundSection">
                <!-- Custom background will be rendered here -->
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="emoji-picker-modal" id="assetsPickerModal" style="display: none;">
          <div class="emoji-picker-content" style="max-width: 900px; max-height: 90vh; width: 90vw;">
            ...
            <div id="customUploadSection" style="display: none; padding: 1.5rem; text-align: center; border-bottom: 1px solid var(--black); background: var(--card-bg); display: flex; flex-direction: column; align-items: center; justify-content: center;">
              ...
              <button onclick="document.getElementById('customAssetUpload').click()" 
                      style="padding: 1rem 2rem; background: var(--primary-color); color: var(--text-primary); border: 2px solid var(--black); border-radius: 8px; cursor: pointer; font-size: 1.125rem; font-weight: 600; transition: all 0.2s; box-shadow: 4px 4px 0 var(--black);"
                      onmouseover="this.style.transform='translate(-2px, -2px)'; this.style.boxShadow='6px 6px 0 var(--black)'"
                      onmouseout="this.style.transform='translate(0, 0)'; this.style.boxShadow='4px 4px 0 var(--black)'"
                      aria-label="Upload custom assets">
                📤 Upload Custom Assets
              </button>
              <p style="margin-top: 1.5rem; font-size: 1rem; color: var(--text-secondary); line-height: 1.6; text-align: center; max-width: 600px;">
...
const MapBuilder = {
  ...
  init() {
    ...
    // Make canvas responsive to wrapper size
    this.resizeCanvas();
    ...
    if (AppState.currentWorld) {
      ...
      // CRITICAL FIX: Reference the same array, don't copy
    ...
    this.canvas.addEventListener('drop', (e) => {
      ...
      if (files && files.length > 0) {
        ...
        if (file.type.startsWith('image/')) {
          // Initialize placedAssets if needed
          if (!AppState.currentWorld.placedAssets) {
            AppState.currentWorld.placedAssets = [];
          }
          ...
          reader.onload = (event) => {
            ...
            this.compressImage(imageData, (compressedData) => {
              ...
              AppState.currentWorld.customAssets.push(compressedData);
              AppState.save();
              ...
              const asset = {
                ...
                height: 128,
                rotation: 0,
                isBuiltIn: false, // Custom uploaded asset
                data: compressedData
              };
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.shiftKey && (e.key === 'D' || e.key === 'd')) {
        ...
        if (this.selectedPlacedAsset) {
          ...
          const duplicatedAsset = {
            id: Date.now(),
            x: this.selectedPlacedAsset.x + 30,
            y: this.selectedPlacedAsset.y + 30,
            width: this.selectedPlacedAsset.width,
            height: this.selectedPlacedAsset.height,
            rotation: this.selectedPlacedAsset.rotation || 0,
            filename: this.selectedPlacedAsset.filename || '',
            isBuiltIn: this.selectedPlacedAsset.isBuiltIn || false,
            data: this.selectedPlacedAsset.data || null
          };
          this.placedAssets.push(duplicatedAsset);
          // placedAssets already references AppState.currentWorld.placedAssets, no need to copy
        ...
        if (this.selectedLabel) {
          const duplicatedLabel = {
            ...
            x: this.selectedLabel.x + 30,
            y: this.selectedLabel.y + 30,
            color: this.selectedLabel.color,
            fontSize: this.selectedLabel.fontSize || 16
          };
          this.saveStateToHistory();
          AppState.currentWorld.mapLabels.push(duplicatedLabel);
          this.selectedLabel = duplicatedLabel;
          AppState.save();
          this.render();
          e.preventDefault();
          return;
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        ...
        if (this.selectedAssets && this.selectedAssets.length > 0) {
          if (confirm(`Delete ${this.selectedAssets.length} selected asset(s)?`)) {
            this.saveStateToHistory();
            this.selectedAssets.forEach(asset => {
              const index = AppState.currentWorld.placedAssets.indexOf(asset);
              if (index > -1) {
                AppState.currentWorld.placedAssets.splice(index, 1);
              }
              const localIndex = this.placedAssets.indexOf(asset);
              if (localIndex > -1) {
                this.placedAssets.splice(localIndex, 1);
              }
            });
            this.selectedAssets = [];
  ...
  handleMouseDown(e) {
    ...
    if (clickedAsset) {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        // Toggle selection
        const index = this.selectedAssets.indexOf(clickedAsset);
        if (index > -1) {
          // Deselect
      ...
      // Single click to select and drag
      this.selectedPlacedAsset = clickedAsset;
      this.selectedAssets = []; // Clear multi-select when single selecting
  ...
  saveDrawing(x1, y1, x2, y2) {
    ...
    // Save state before first drawing stroke in a session
    if (!this.drawingSessionStarted) {
      this.saveStateToHistory();
  ...
  openBackgroundPicker() {
    ...
    if (modal) {
      ...
      this.renderCustomBackgroundSection();
    }
  },
  ...
  renderCustomBackgroundSection() {
    const section = document.getElementById('customBackgroundSection');
    if (!section) return;
    
    const customBg = AppState.currentWorld.customBackground;
    
    if (customBg) {
      // Show custom background with preview and remove button
      section.innerHTML = `
        <div style="display: flex; flex-direction: column; gap: 1rem;">
          <button class="background-option" onclick="MapBuilder.setBackground('custom')" style="width: 100%;">
            <div class="background-preview" style="background-image: url('${customBg}'); background-size: cover; background-position: center;"></div>
            <span>Custom Background</span>
          </button>
          <button class="btn btn-danger" onclick="MapBuilder.removeCustomBackground()" style="width: 100%;">
      ...
      section.innerHTML = `
        <div style="display: flex; flex-direction: column; gap: 0.5rem;">
          <input type="file" id="customBackgroundInput" accept="image/*" style="display: none;" onchange="MapBuilder.handleCustomBackgroundUpload(event)">
          <button class="btn btn-secondary" onclick="document.getElementById('customBackgroundInput').click()" style="width: 100%;">
            📤 Upload Custom Background
          </button>
          <p style="font-size: 0.75rem; color: var(--text-secondary); margin: 0; text-align: center;">Upload your own background image</p>
        </div>
      `;
    }
  },
  ...
  handleCustomBackgroundUpload(event) {
    ...
    reader.onload = (e) => {
      ...
      // Compress the image for background (larger size, high quality)
      this.compressImage(imageData, (compressedData) => {
        // Save to AppState
        AppState.currentWorld.customBackground = compressedData;
        AppState.save();
        
        // Automatically set as current background
        this.setBackground('custom');
        
        // Re-render the custom background section
        this.renderCustomBackgroundSection();
        
        // Clear the file input
        event.target.value = '';
      }, 1920, 0.85); // Max 1920px, 85% quality for backgrounds
    };
    reader.readAsDataURL(file);
  },
  ...
  removeCustomBackground() {
    ...
    delete AppState.currentWorld.customBackground;
    
    // Clear cached image
    if (this.backgroundImages && this.backgroundImages['custom']) {
      delete this.backgroundImages['custom'];
    }
  ...
  handleCustomAssetUpload(event) {
    ...
    // Check custom asset limit (20 custom assets max)
    const MAX_CUSTOM_ASSETS = 20;
    const currentCount = AppState.currentWorld.customAssets.length;
    const newCount = currentCount + files.length;
    
    if (newCount > MAX_CUSTOM_ASSETS) {
      alert(`⚠️ Custom Asset Limit\n\nYou can upload a maximum of ${MAX_CUSTOM_ASSETS} custom assets per world.\n\nCurrent: ${currentCount}\nTrying to add: ${files.length}\nLimit: ${MAX_CUSTOM_ASSETS}\n\nPlease remove some custom assets first or select fewer files.`);
      return;
    }
    ...
    Array.from(files).forEach(file => {
      ...
      reader.onload = (e) => {
        ...
        this.compressImage(imageData, (compressedData) => {
          ...
          // Save and reload after all files are processed
  ...
  loadAssetsForTheme(theme) {
    ...
    const assetFiles = {
      ...
      normal: [
        ...
        'normal/nature/oak_tree.png', 'normal/nature/pine_tree.png', 'normal/nature/willow_tree.png', 'normal/nature/maple_tree.png', 
        'normal/nature/round_bush.png', 'normal/nature/flowering_bush.png', 'normal/nature/hedge_bush.png', 'normal/nature/wild_bush.png', 
        'normal/vehicles/blimp.png'
      ],
      modern: [
    ...
    files.forEach(filename => {
      ...
      assetBtn.appendChild(img);
      assetBtn.onclick = () => this.selectAsset(assetPath, assetPath); // Pass assetPath as filename too
  ...
  placeAsset(x, y) {
    ...
    // Save state before placing asset
    this.saveStateToHistory();
    ...
    const asset = {
      ...
      isBuiltIn: !!this.selectedAsset.filename, // true if it has a filename (built-in asset)
      data: this.selectedAsset.filename ? null : this.selectedAsset.data // Only store data for custom uploads
    };
  ...
  duplicateAsset(originalAsset) {
    ...
    const duplicate = {
      ...
      isBuiltIn: originalAsset.isBuiltIn || false,
      data: originalAsset.data || null
    };
  ...
  updateAssetCounter() {
    ...
    if (assetCountText) {
      const count = AppState.currentWorld.placedAssets ? AppState.currentWorld.placedAssets.length : 0;
  ...
  drawAssetOnCanvas(asset, img) {
    ...
    // Draw multi-select indicator (green) if this asset is in selectedAssets array
    if (this.selectedAssets && this.selectedAssets.includes(asset)) {
    ...
    else if (this.selectedPlacedAsset === asset) {
  ...
  drawBackground() {
    ...
    if (this.currentBackground === 'custom' && AppState.currentWorld.customBackground) {
      const customBg = AppState.currentWorld.customBackground;
      
      if (this.backgroundImages && this.backgroundImages['custom']) {
        // Use cached custom background
        ...
        // Load and cache custom background
    ...
    const backgrounds = {
      ...
      'newland': 'assets/backgrounds/newland.png',
      'winterland': 'assets/backgrounds/winterland.png',
      'town': 'assets/backgrounds/town.png',
      'darkfantasy': 'assets/backgrounds/darkfantasy.png',
      'ruraltown': 'assets/backgrounds/ruraltown.png',
      'oldcity': 'assets/backgrounds/oldcity.png'
    };
    
    const bgTexture = backgrounds[this.currentBackground];
    
    if (bgTexture && this.backgroundImages && this.backgroundImages[this.currentBackground]) {
  ...
  saveStateToHistory() {
    ...
    const state = {
      mapNodes: JSON.parse(JSON.stringify(AppState.currentWorld.mapNodes || [])),
      mapDrawings: JSON.parse(JSON.stringify(AppState.currentWorld.mapDrawings || [])),
      placedAssets: JSON.parse(JSON.stringify(AppState.currentWorld.placedAssets || [])),
      mapLabels: JSON.parse(JSON.stringify(AppState.currentWorld.mapLabels || [])),
      mapBackground: this.currentBackground
    };
  ...
  undo() {
    ...
    AppState.currentWorld.mapNodes = previousState.mapNodes;
    AppState.currentWorld.mapDrawings = previousState.mapDrawings;
    AppState.currentWorld.placedAssets = previousState.placedAssets;
    AppState.currentWorld.mapLabels = previousState.mapLabels;
    this.currentBackground = previousState.mapBackground;
  ...
  render() {
    ...
    if (AppState.currentWorld.placedAssets) {
      ...
      AppState.currentWorld.placedAssets.forEach(asset => {
        // Determine the image source (filename for built-in, data for custom)
        const imgSrc = asset.isBuiltIn ? asset.filename : asset.data;
        if (!imgSrc) return; // Skip if no source
        
        // Check cache first
        let img = this.assetImageCache.get(imgSrc);
        ...
        if (img && img.complete) {
          ...
          img = new Image();
          img.onload = () => {
            this.assetImageCache.set(imgSrc, img);
            this.drawAssetOnCanvas(asset, img);
          };
          img.src = imgSrc;
        }
    ...
    modal.innerHTML = `
      <div style="background: var(--card-bg); border-radius: 12px; padding: 2rem; width: 90%; max-width: 500px; box-shadow: 0 20px 60px rgba(0,0,0,0.3);">
        ...
        <div style="display: flex; gap: 0.75rem; justify-content: flex-end;">
          <button onclick="this.closest('div[style*=\"position: fixed\"]').remove()" style="padding: 0.75rem 1.5rem; background: var(--bg-secondary); color: var(--text-primary); border: 2px solid var(--border-color); border-radius: 6px; cursor: pointer; font-weight: 600;">Cancel</button>
          <button onclick="HierarchyTree.saveNodeEdit('${nodeId}')" style="padding: 0.75rem 1.5rem; background: var(--text-primary); color: var(--bg-primary); border: 2px solid var(--border-color); border-radius: 6px; cursor: pointer; font-weight: 600;">Save Changes</button>
        </div>
      </div>
    `;
...
const Modal = {
  ...
  showEditLocation(id) {
    ...
    this.show(
      ...
      `
        <button class="btn btn-secondary" onclick="Modal.close()">Cancel</button>
        <button class="btn btn-secondary" onclick="Modal.duplicateLocation('${id}')">Duplicate</button>
  ...
  showEditCharacter(id) {
    ...
    this.show(
      ...
      `
        <button class="btn btn-secondary" onclick="Modal.close()">Cancel</button>
        <button class="btn btn-secondary" onclick="Modal.duplicateCharacter('${id}')">Duplicate</button>
  ...
  showEditFaction(id) {
    ...
    this.show(
      ...
      `
        <button class="btn btn-secondary" onclick="Modal.close()">Cancel</button>
        <button class="btn btn-secondary" onclick="Modal.duplicateFaction('${id}')">Duplicate</button>
  ...
  showEditEvent(id) {
    ...
    this.show(
      ...
      `
        <button class="btn btn-secondary" onclick="Modal.close()">Cancel</button>
        <button class="btn btn-secondary" onclick="Modal.duplicateEvent('${id}')">Duplicate</button>
  ...
  showEditRule(id) {
    ...
    this.show(
      ...
      `
        <button class="btn btn-secondary" onclick="Modal.close()">Cancel</button>
        <button class="btn btn-secondary" onclick="Modal.duplicateRule('${id}')">Duplicate</button>
  ...
  showEditTheme(id) {
    ...
    this.show(
      ...
      `
        <button class="btn btn-secondary" onclick="Modal.close()">Cancel</button>
        <button class="btn btn-secondary" onclick="Modal.duplicateTheme('${id}')">Duplicate</button>
  ...
  showEditReligion(id) {
    ...
    `, `<button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-secondary" onclick="Modal.duplicateReligion('${id}')">Duplicate</button><button class="btn btn-primary" onclick="Modal.updateReligion('${id}')">Save Changes</button></div>`);
  },
  updateReligion(id) {
    const r=AppState.currentWorld.religions.find(x=>x.id===id);if(!r)return;
  ...
  showEditFashion(id) {
    ...
    `, `<button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-secondary" onclick="Modal.duplicateFashion('${id}')">Duplicate</button><button class="btn btn-primary" onclick="Modal.updateFashion('${id}')">Save Changes</button></div>`);
  },
  updateFashion(id) {
    const f=AppState.currentWorld.fashions.find(x=>x.id===id);if(!f)return;
  ...
  showEditMagicSystem(id) {
    ...
    `, `<button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-secondary" onclick="Modal.duplicateMagicSystem('${id}')">Duplicate</button><button class="btn btn-primary" onclick="Modal.updateMagicSystem('${id}')">Save Changes</button></div>`);
  },
  ...
  showEditLanguage(id) {
    ...
    \`, \`<button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updateLanguage('${id}')">Save Changes</button></div>`);
  },
  updateLanguage(id) {
    const l=AppState.currentWorld.languages.find(x=>x.id===id);if(!l)return;
  ...
  showEditEconomy(id) {
    ...
    `, `<div style="display: flex; justify-content: flex-end; gap: 0.75rem;"><button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updateEconomy('${id}')">Save Changes</button></div>`);
  },
  updateEconomy(id) {
    const e=AppState.currentWorld.economies.find(x=>x.id===id);if(!e)return;
  ...
  showEditPolitics(id) {
    ...
    \`, \`<button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updatePolitics('${id}')">Save Changes</button></div>`);
  },
  updatePolitics(id) {
    const p=AppState.currentWorld.politics.find(x=>x.id===id);if(!p)return;
  ...
  showEditTechnology(id) {
    ...
    `, `<div style="display: flex; justify-content: flex-end; gap: 0.75rem;"><button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updateTechnology('${id}')">Save Changes</button></div>`);
  },
  updateTechnology(id) {
    const t=AppState.currentWorld.technologies.find(x=>x.id===id);if(!t)return;
  ...
  showEditFloraFauna(id) {
    ...
    `, `<div style="display: flex; justify-content: flex-end; gap: 0.75rem;"><button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updateFloraFauna('${id}')">Save Changes</button></div>`);
  },
  updateFloraFauna(id) {
    const f=AppState.currentWorld.floraFauna.find(x=>x.id===id);if(!f)return;
  ...
  showEditCreature(id) {
    ...
    `, `<div style="display: flex; justify-content: flex-end; gap: 0.75rem;"><button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updateCreature('${id}')">Save Changes</button></div>`);
  },
  updateCreature(id) {
    const c = AppState.currentWorld.creatures.find(x => x.id === id);
    if (!c) return;
  ...
  showEditCuisine(id) {
    ...
    `, `<div style="display: flex; justify-content: flex-end; gap: 0.75rem;"><button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updateCuisine('${id}')">Save Changes</button></div>`);
  },
  updateCuisine(id) {
    const c=AppState.currentWorld.cuisines.find(x=>x.id===id);if(!c)return;
  ...
  showEditArtifact(id) {
    ...
    `, `<div style="display: flex; justify-content: flex-end; gap: 0.75rem;"><button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updateArtifact('${id}')">Save Changes</button></div>`);
  },
  updateArtifact(id) {
    const a=AppState.currentWorld.artifacts.find(x=>x.id===id);if(!a)return;
  ...
  showAddSocialInteraction() {
    ...
    \`, \`<button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.saveSocialInteraction()">Add Interaction</button></div>`);
  },
  ...
  showEditSocialInteraction(id) {
    ...
    \`, \`<button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updateSocialInteraction('${id}')">Save Changes</button></div>`);
  },
  ...
  showEditHierarchy(id) {
    ...
    `, `<div style="display: flex; justify-content: flex-end; gap: 0.75rem;"><button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updateHierarchy('${id}')">Save Changes</button></div>`);
  },
  updateHierarchy(id) {
    const hier=AppState.currentWorld.hierarchies.find(x=>x.id===id);if(!hier)return;
  ...
  showEditEtiquette(id) {
    ...
    this.show('Edit Etiquette System', `
     ...
     `, `<div style="display: flex; justify-content: flex-end; gap: 0.75rem;"><button class="btn btn-secondary" onclick="Modal.close()">Cancel</button><button class="btn btn-primary" onclick="Modal.updateEtiquette('${id}')">Save Changes</button></div>`);
  },
  updateEtiquette(id) {
    const etiq=AppState.currentWorld.etiquette.find(x=>x.id===id);if(!etiq)return;
